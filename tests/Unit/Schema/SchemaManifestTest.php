...
namespace Glueful\Extensions\StorageAzure\Tests\Unit\Schema;
...
final class SchemaManifestTest extends TestCase
{
    ...
    {
        $base = sys_get_temp_dir() . '/storage-azure-mf-' . uniqid('', true);
        ...
        self::assertSame([], $manifest->migrationDescriptors()['glueful/storage-azure']);
        self::assertNotContains('glueful/storage-azure', $manifest->undeclaredGluefulPackages());
    }

    public function testResolvesWhenEnabledAloneAtItsDeclaredFloor(): void
    {
        $g = $this->manifest();
        $candidates = ['glueful/storage-azure' => new \Glueful\Extensions\ExtensionCandidate(
            name: 'glueful/storage-azure',
            provider: $g['provider'],
